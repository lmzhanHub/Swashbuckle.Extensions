    ...
    public class SwaggerGenerator : ISwaggerProvider
    {
        ...
        public SwaggerDocument GetSwagger(string rootUrl, string apiVersion)
        {
            ...
            var paths = GetApiDescriptionsFor(apiVersion)
                .Where(apiDesc => !(_options.IgnoreObsoleteActions && apiDesc.IsObsolete()) && !(_options.IgnoreControllers && apiDesc.IsIgnoreController()))
                .OrderBy(_options.GroupingKeySelector, _options.GroupingKeyComparer)
                .GroupBy(apiDesc => apiDesc.RelativePathSansQueryString())
                .ToDictionary(group => "/" + (_options.RelativePathResolver!=null? _options.RelativePathResolver(group.Key):group.Key), group => CreatePathItem(group, schemaRegistry));
        ...
        {
            var parameters = (_options.IgnoreParamters != null? apiDesc.ParameterDescriptions.Where(p => !_options.IgnoreParamters(p.Name)):apiDesc.ParameterDescriptions)
                .Select(paramDesc =>
                    {
